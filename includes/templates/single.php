...
if ( is_user_logged_in() && isset($_POST['suscribed_list']) ){
	...
	$postID = get_the_ID();
    $interesados = get_post_meta($postID ,'usuarios_interesados', true);
    $userID = get_current_user_id();
	$user_insert = $userID . ',';
	$user_interests = get_user_meta($userID, 'interesado_actividades', true);
	$post_insert = $postID . ',';
	
	if ( $_POST['suscribed_list'] == 'add' && strpos($interesados, $user_insert) === false) {
	    $interesados = $interesados . $user_insert; 
	    update_post_meta($postID, 'usuarios_interesados', $interesados);
	    $user_interests = $user_interests . $post_insert;
	    update_user_meta( $userID, 'interesado_actividades', $user_interests );
	}
	if ( $_POST['suscribed_list'] == 'remove' && strpos($interesados, $user_insert) !== false) {
        $interesados = str_replace($user_insert, '', $interesados); 
        update_post_meta($postID, 'usuarios_interesados', $interesados);
        $user_interests = str_replace( $post_insert ,'', $user_interests );
	    update_user_meta( $userID, 'interesado_actividades', $user_interests );
	}
}
...
<div id="primary" class="content-area">
    ...
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBjeU9-8sUIfBFeM5PrwrdkonI0-hdIGQ4&callback=initMap">
            ...
            <?php
			    ...
			    $interesados = get_post_meta($postID ,'usuarios_interesados', true);
			    $is_interested = strpos( $interesados, $userID . ',' ) === false;
			    $user_interests = get_user_meta($userID, 'interesado_actividades', true); ?>
			    <div class="suscribe-button">
                	<form class="suscribe-form" action="" method="post"> 
						...
						<?php wp_nonce_field( 'form_actividad','nonce_form' ); ?>
                        <input type="submit" value="<?php echo ($is_interested ) ? 'Me interesa' : 'Ya no me intresa'; ?>" />
                        <input type="hidden" name="suscribed_list" value="<?php echo ($is_interested) ? 'add' : 'remove'; ?>" />
                    </form>
                </div><?php 
			}
			?>
